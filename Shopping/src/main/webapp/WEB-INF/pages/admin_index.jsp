    ...
    <section id="content_wrapper">
      ...
      <div style="padding-top: 30px">
        <div style="float: left">
          <form class="form-inline" action="${pageContext.request.contextPath}/admin/toAdminIndex" method="post">
            <div class="form-group">
              <label style="margin-left: 30px" >商品名称</label>
              <input type="text" class="form-control" value="${map.cname}" name="cname" id="cname">
            </div>
            ...
            <button type="submit" class="btn btn-default" style="margin-right: 30px">查询</button>
          </form>
        </div>
        ...
        <div style="float: left;margin-bottom: 5px" >
          <tr>
            &nbsp;&nbsp;&nbsp;<td colspan="8" align="center"><a href="${pageContext.request.contextPath}/admin/toAddPageForCommodity" class="btn btn-primary">新增商品</a></td>
          </tr>
        </div>
        ...
        <div style="float: left;margin-bottom: 5px" >
          <tr>
            &nbsp;&nbsp;&nbsp;<td colspan="8" align="center"><a href="${pageContext.request.contextPath}/admin/toAddPageForCategory" class="btn btn-primary">新增类别</a></td>
          </tr>
        </div>
      </div>
        ...
        <div class="col-md-12">
          <div class="panel panel-visible" id="spy2">
            ...
            <div class="panel-body pn">
              <form id="form" action="${pageContext.request.contextPath}/admin/deleteCounts" method="post">
              ...
              <table class="table table-striped table-hover" id="datatable2" cellspacing="0" width="100%">
                ...
                <tbody>
                  ...
                  <c:forEach items="${pageInfo.list}" var="commodity" varStatus="x">
                    <tr>
                      <td><input type="checkbox" name="nid" value="${commodity.id}"></td>
                      <td>${x.count+(pageInfo.pageNum-1) * pageInfo.pageSize}</td>
                      <td><img style="width: 80px" src="${pageContext.request.contextPath}${commodity.photoUrl}"></td>
                      <td>${commodity.name}</td>
                      <td>${commodity.category}</td>
                      <td>${commodity.price}</td>
                      <c:if test="${commodity.discount != 1}">
                        <td>${commodity.discount * 10}折</td>
                      </c:if>
                      <c:if test="${commodity.discount == 1}">
                        <td>未打折</td>
                      </c:if>
                      <td>${commodity.salesVolume}</td>
                      <td>${commodity.inventory}</td>
                      <td>${commodity.status eq 0?"下架":"在售"}</td>
                      <td>
                        <a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/admin/changeStateById?id=${commodity.id}&status=${commodity.status}">${commodity.status eq 0?"上架":"下架"}</a>&nbsp;
                        <a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/admin/findCommodityByCid?cid=${commodity.id}">商品详情</a>
                      </td>
                    </tr>
                  </c:forEach>
                </tbody>
                ...
                <nav aria-label="Page navigation">
                  <ul class="pagination">
                    ...
                    <li>
                      ...
                      <a href="${pageContext.request.contextPath}/admin/toAdminIndex?pageNum=${pageInfo.pageNum-1}&pageSize=${pageInfo.pageSize}&name=${map.name}&address=${map.address}&email=${map.email}" aria-label="Previous">
                    ...
                    <c:forEach begin="1" end="${pageInfo.pages}" var="i">
                      ...
                      <c:if test="${pageInfo.pageNum==i}">
                        <li class="active"><a href="${pageContext.request.contextPath}/admin/toAdminIndex?pageNum=${i}&pageSize=${pageInfo.pageSize}&name=${map.name}&address=${map.address}&email=${map.email}">${i}</a></li>
                      </c:if>

                      <c:if test="${pageInfo.pageNum!=i}">
                        <li><a href="${pageContext.request.contextPath}/admin/toAdminIndex?pageNum=${i}&pageSize=${pageInfo.pageSize}&name=${map.name}&address=${map.address}&email=${map.email}">${i}</a></li>
                      </c:if>
                    </c:forEach>
                    ...
                    <li>
                      ...
                      <a href="${pageContext.request.contextPath}/admin/toAdminIndex?pageNum=${pageInfo.pageNum+1}&pageSize=${pageInfo.pageSize}&name=${map.name}&address=${map.address}&email=${map.email}" aria-label="Next">
                    ...
                    <div style="float: left">
                      <li class="threeword">
                        <%-- &lt;%&ndash;改变下拉框(每页条数) 触发js事件,重新发起分页请求--%>
                        <select id="province" class="threeword" style="height: 35px" onchange="changeSelect();">
                          <c:if test="${pageInfo.pageSize == 5}">
                            <option value="5" style="color: blue" selected>5条/页</option>
                          ...
                          <c:if test="${pageInfo.pageSize == 10}">
                            ...
                            <option value="10" style="color: blue" selected>10条/页</option>
                            <option value="15">15条/页</option>
                            <option value="20">20条/页</option>
                          </c:if>
                          <c:if test="${pageInfo.pageSize == 15}">
                            <option value="5" >5条/页</option>
                            <option value="10">10条/页</option>
                            <option value="15" style="color: blue" selected>15条/页</option>
                            <option value="20">20条/页</option>
                          </c:if>
                          <c:if test="${pageInfo.pageSize == 20}">
                            ...
                            <option value="10">10条/页</option>
                            <option value="15">15条/页</option>
                            <option value="20" style="color: blue" selected>20条/页</option>
                          </c:if>
                        </select>
                      </li>
                    </div>
