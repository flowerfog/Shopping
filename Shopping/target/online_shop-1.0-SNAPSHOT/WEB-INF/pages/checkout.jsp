...
<!-- /NAVIGATION -->
		...
		<div class="section">
			<!-- container -->
			<div class="container">
				<form name="form1" method="post" action="${pageContext.request.contextPath}/order/confirmOrder">
				<!-- row -->
				<div class="row">
					...
					<div class="col-md-7">
						...
						<div class="shiping-details">
							...
							<div class="input-checkbox">
								...
								<c:forEach items="${addressList}" var="address" varStatus="x">
									...
									<div class="caption">
										<div class="form-group">
											<input class="input" type="hidden" name="aid" value="${address.id}">
										</div>
										<div class="form-group">
											<input class="input" readonly="readonly" type="text" name="name" placeholder="收货人 (Consignee)" value="${address.name}">
										</div>
										<div class="form-group">
											<input class="input"  readonly="readonly"  type="text" name="phone" placeholder="手机号 (phone number)" value="${address.phone}">
										</div>
										<div class="form-group">
											<input class="input" readonly="readonly"  type="email" name="email" placeholder="邮箱 (Email) 选填" value="${address.email}">
										</div>
										<div class="form-group">
											<input class="input" readonly="readonly"  type="text" name="city" placeholder="所在地区 省、市、区、街区 (Province, City, District, Block)" value="${address.city}">
										</div>
										<div class="form-group">
											<input class="input" readonly="readonly"  type="text" name="detail" placeholder="详细地址 (Detail)" value="${address.detail}">
										</div>
									</div>
								</c:forEach>
							</div>
						...
						<div class="order-notes">
							<textarea class="input" placeholder="备注 (Order Notes)"></textarea>
						</div>
						<!-- /Order notes -->
					</div>
					...
					<div class="col-md-5 order-details">
						...
						<div class="order-summary">
							<div class="order-col">
								<div><strong>宝贝名称</strong></div>
								<div><strong>宝贝单价</strong></div>
							</div>
							<div class="order-products">
								<c:forEach items="${commodityOfOrder}" var="shoppingCart" varStatus="x" >
									<div class="order-col">
										<div><strong style="color: darkgreen">${shoppingCart.commodityCount}</strong> x ${shoppingCart.commodity.name}</div>
										<div>￥ ${shoppingCart.commodity.price * shoppingCart.commodity.discount}</div>
									</div>
								</c:forEach>
							...
							<div class="order-col">
								<div><strong>订单总价</strong></div>
								<div><strong class="order-total">￥ ${orderPrice}</strong></div>
							</div>
						</div>
						<div class="payment-method">
							<div class="input-radio">
								<input type="radio" name="payment" id="payment-1" value="balance" checked>
								<label for="payment-1">
									<span></span>
									余额支付
								</label>
								<div class="caption">
									<c:if test="${sessionScope.user.money < orderPrice}">
										<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您当前用户余额不足,请<a href="#" style="color: red"><b>充值</b></a>后消费</p>
									</c:if>

									<c:if test="${sessionScope.user.money >= orderPrice}">
										<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您当前用户余额充足,<span style="color: green"><b>放心购买</b></span></p>
									</c:if>
								</div>
							</div>
							<div class="input-radio">
								...
								<div class="caption">
									<img style="width: 300px" src="${pageContext.request.contextPath}/img/chat.png">
								</div>
							</div>
							<div class="input-radio">
						...
						<a href="javascript:document.form1.submit();" class="primary-btn order-submit">提交订单</a>
					</div>
					<!-- /Order Details -->
				</div>
				<!-- /row -->
